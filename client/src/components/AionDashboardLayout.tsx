...
export default function AionDashboardLayout({ children }: AionDashboardLayoutProps) {
  ...
  return (
    <SidebarProvider open={sidebarOpen} onOpenChange={handleSidebarOpenChange}>
      ...
      <Dialog open={clientDataOpen} onOpenChange={setClientDataOpen}><DialogContent><DialogHeader><DialogTitle>Meus dados cadastrais</DialogTitle><DialogDescription>Consulte seus dados. Para qualquer alteração, solicite atendimento ao suporte Aion.</DialogDescription></DialogHeader><dl className="mt-5 grid gap-3 text-sm"><div><dt className="text-muted-foreground">Nome</dt><dd className="font-medium text-foreground">{linkedClient?.name ?? "—"}</dd></div><div><dt className="text-muted-foreground">E-mail</dt><dd className="font-medium text-foreground">{linkedClient?.email ?? "—"}</dd></div><div><dt className="text-muted-foreground">Telefone</dt><dd className="font-medium text-foreground">{linkedClient?.phone ?? "—"}</dd></div><div><dt className="text-muted-foreground">Endereço físico</dt><dd className="font-medium text-foreground">{linkedClient?.address ?? "—"}</dd></div></dl><DialogFooter className="mt-5"><Button onClick={() => setClientDataOpen(false)}>Fechar</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={supportOpen} onOpenChange={setSupportOpen}><DialogContent><DialogHeader><DialogTitle>Solicitar alteração cadastral</DialogTitle><DialogDescription>Para preservar a consistência financeira do seu atendimento, os dados cadastrais são atualizados pelo suporte Aion mediante solicitação.</DialogDescription></DialogHeader><DialogFooter className="mt-5"><Button onClick={() => setSupportOpen(false)}>Entendi</Button></DialogFooter></DialogContent></Dialog>
    </SidebarProvider>
  );
}
